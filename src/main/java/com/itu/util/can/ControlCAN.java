...
import com.itu.util.can.UsbDriver;
import com.itu.util.can.UserType;
...
public class ControlCAN {
    ...
    public volatile boolean CanReaderExitFlag = false;
        ...
        public void run() {
            ...
            while (!ControlCAN.this.Can1ExitFlag) {
                ...
                synchronized (e2) {
                    ...
                    byte status;
                    try {
                    	status = ControlCAN.this.USB_Driver.USB_GetStatus();
                    } catch (LibUsbException e) {
                    	System.out.println(e.getMessage());
                    	System.out.println("Don't worry if it's caused by timeout");
                    	continue;
                    }
                    if (status != 0) {
                        short s2 = this.i = 0;
